...
from keras.preprocessing.image import img_to_array, array_to_i
...
print('important indexes')
L1=sorted(np.load('~/proposed_pruning/sim_index1.npy')[64-len(L1):64])
L2=sorted(np.load('~/proposed_pruning/sim_index2.npy')[64-len(L2):64])#np.arange(0,64)#
L3=sorted(np.load('~/proposed_pruning/sim_index3.npy')[128-len(L3):128])
L4=sorted(np.load('~/proposed_pruning/sim_index4.npy')[128-len(L4):128])
L5=sorted(np.load('~/proposed_pruning/sim_index5.npy')[256-len(L5):256])
L6=sorted(np.load('~/proposed_pruning/sim_index6.npy')[256-len(L6):256])
L7=sorted(np.load('~/proposed_pruning/sim_index7.npy')[256-len(L7):256])#[64:]
L8=sorted(np.load('~/proposed_pruning/sim_index8.npy')[512-len(L8):512])
L9=sorted(np.load('~/proposed_pruning/sim_index9.npy')[512-len(L9):512])#[256:]
L10=sorted(np.load('~/proposed_pruning/sim_index10.npy')[512-len(L10):512])#[256:]
L11=sorted(np.load('~/proposed_pruning/sim_index11.npy')[512-len(L11):512])#[256:]
L12=sorted(np.load('~/proposed_pruning/sim_index12.npy')[512-len(L12):512])#[256:]
L13=sorted(np.load('~/proposed_pruning/sim_index13.npy')[512-len(L13):512])#[256:]
...
W=np.load('~/VGG_MNIST/baseline_200/best_weights_numpy.npy',allow_pickle=True) # unpruned trained weights
...
#%%data prepration, converting graysacle images to RGB.
...
checkpointer = ModelCheckpoint(filepath='~/best_weights_1.h5py',monitor='val_acc',verbose=1, save_best_only=True,save_weights_only=True)
hist=model.fit(X_train,y_train,batch_size=128,epochs=100,verbose=1,validation_data=(X_test,y_test),callbacks=[checkpointer])

model.load_weights('~/best_weights_1.h5py')

Z=model.get_weights()
np.save('~/best_weights_numpy_1.npy',Z)



model.save('~/best_model_VGG_MNIST_1.h5')
